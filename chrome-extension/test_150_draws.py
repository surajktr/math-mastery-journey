...
def load_data(filepath, limit=150):
    with open(filepath, 'r', encoding='utf-8') as f:
        data = json.load(f)
    # The JSON usually has newest draws first.
    # The JS extension does: history.slice(0, 100).reverse()
    # So we slice the first `limit` records and reverse them so oldest is first.
    subset = data[:limit]
    subset.reverse()
...
def simulate_strategy(data, direction, shield_limit, tp, tp_pause, sl, sl_pause, run, pause_time, seq=[10, 20, 40], initial_balance=100):
...
def run_grid_search(filepath):
    try:
        data = load_data(filepath, limit=50)
    ...
    tps = [20, 50, 100, 150, 200]
    tp_pauses = [0, 1, 2, 5]
    sls = [0, 20, 30, 40, 50, 80]
    sl_pauses = [0, 1, 2, 3]
    runs = [0, 15, 30]
    pauses = [0, 2, 5]
    ...
    total = len(directions)*len(sequences)*len(shield_limits)*len(tps)*len(tp_pauses)*len(sls)*len(sl_pauses)*len(runs)*len(pauses)
                            ...
                            for slp in sl_pauses:
                                for run in runs:
                                    for pause in pauses:
                                        ...
                                        profit = simulate_strategy(data, direction, shield, tp, tpp, sl, slp, run, pause, seq=seq)
    ...
    print(f"Staking Sequence: {best_config['seq']}")
...
if __name__ == '__main__':
    run_grid_search(r'C:\Users\Suraj\Downloads\wingo_300_history (19).json')
